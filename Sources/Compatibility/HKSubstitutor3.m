...
enum { HK3_LEGACY_IMAGE_MAGIC = 0x484B3349u };  // "HK3I"
...
typedef NS_ENUM(uint8_t, HK3LegacyTechnique) {
    HK3LegacyTechniqueNone,
};

// This is a public ABI detail of 2.x: do not add fields.  The actual HK3
...
typedef enum {
    HK3_LEGACY_OP_MESSAGE,
    HK3_LEGACY_OP_FUNCTION,
    HK3_LEGACY_OP_MEMORY,
} hk3_legacy_operation_kind_t;

@interface HK3LegacyOperation : NSObject {
@public
    hk3_legacy_operation_kind_t kind;
    Class objcClass;
    SEL selector;
    void *target;
    void *replacement;
...
@implementation HK3LegacyOperation
@end

static bool hk3_legacy_capture_image(void *opaque, size_t index,
                                     const hk_image_entry_t *entry) {
    (void)index;
    hk3_legacy_image_t *image = opaque;
...
static bool hk3_legacy_find_image(void *opaque, size_t index,
                                  const hk_image_entry_t *entry) {
    (void)index;
    struct {
        const char *name;
...
static void *hk3_legacy_find_symbol_in_payload(const hk3_legacy_image_t *image,
                                                const char *name) {
...
- (id)hk_backendForAutoCoverFunction:(void *)function
                      ...
                      requestOriginal:(BOOL)requestOriginal
                            technique:(HK3LegacyTechnique *)technique;
- (id)hk_backendForAutoCoverFunction:(void *)function
                           replacement:(void *)replacement
                       requestOriginal:(BOOL)requestOriginal
...
@implementation HKSubstitutor {
    ...
    BOOL _batching;
    NSMutableArray<HK3LegacyOperation *> *_queuedOperations;
    int _lastError;
}
...
- (hookkit_status_t)executeOperation:(HK3LegacyOperation *)operation onBackend:(id)backend {
    (void)backend;
    switch (operation->kind) {
    case HK3_LEGACY_OP_MESSAGE:
        return (hookkit_status_t)hk3_legacy_hook_objc((__bridge void *)operation->objcClass,
                                                       (void *)operation->selector,
                                                       operation->replacement,
                                                       operation->oldPtr);
    case HK3_LEGACY_OP_FUNCTION:
        return (hookkit_status_t)hk3_legacy_hook_function(operation->target,
                                                           operation->replacement,
                                                           operation->oldPtr);
    case HK3_LEGACY_OP_MEMORY:
        return (hookkit_status_t)hk3_legacy_hook_memory(operation->target,
...
- (void)drainGroup:(NSArray *)hooks onBackend:(id)backend {
    for (HK3LegacyOperation *operation in hooks) {
...
- (hookkit_status_t)hookMessageInClass:(Class)objcClass
    ...
    if (_batching) {
        HK3LegacyOperation *operation = [HK3LegacyOperation new];
        operation->kind = HK3_LEGACY_OP_MESSAGE;
    ...
    hookkit_status_t status = (hookkit_status_t)hk3_legacy_hook_objc(
...
- (hookkit_status_t)hookFunction:(void *)function
    ...
    if (_batching) {
        HK3LegacyOperation *operation = [HK3LegacyOperation new];
        operation->kind = HK3_LEGACY_OP_FUNCTION;
    ...
    hookkit_status_t status = (hookkit_status_t)hk3_legacy_hook_function(
...
- (hookkit_status_t)hookMemory:(void *)target
    ...
    if (_batching) {
        HK3LegacyOperation *operation = [HK3LegacyOperation new];
        operation->kind = HK3_LEGACY_OP_MEMORY;
    ...
    hookkit_status_t status = (hookkit_status_t)hk3_legacy_hook_memory(target, data, size);
    [self noteHookResult:status];
    return status;
}
...
- (hookkit_status_t)hookSwiftMethodInClass:(Class)objcClass
    ...
    hookkit_status_t status = (hookkit_status_t)hk3_legacy_hook_swift_method(
...
- (hookkit_status_t)hookSwiftVtableSlotInClass:(Class)objcClass
    ...
    hookkit_status_t status = (hookkit_status_t)hk3_legacy_hook_swift_slot(
...
- (HKImageRef)openImage:(NSString *)path {
    ...
    hk3_legacy_image_t *payload = calloc(1, sizeof(*payload));
    ...
    (void)hk_image_catalog_match(runtime->catalog, &selector,
                                 hk3_legacy_capture_image, payload);
    if (!payload->header) {
        hk_runtime_release(runtime);
        free(payload);
        free(image);
    ...
    image->magic = HK3_LEGACY_IMAGE_MAGIC;
...
- (void)closeImage:(HKImageRef)opaqueImage {
    ...
    if (!image || image->magic != HK3_LEGACY_IMAGE_MAGIC) {
        return;
    }
    hk3_legacy_image_t *payload = image->rawHandle;
    image->magic = 0;
    if (payload) {
...
- (void *)findSymbolInImage:(HKImageRef)opaqueImage symbolName:(NSString *)symbolName {
    ...
    if (image) {
        if (image->magic != HK3_LEGACY_IMAGE_MAGIC) {
            return NULL;
        }
        return hk3_legacy_find_symbol_in_payload(image->rawHandle, name);
    }
...
- (hookkit_status_t)executeHooks {
    NSArray<HK3LegacyOperation *> *operations;
    @synchronized(self) {
    ...
    for (HK3LegacyOperation *operation in operations) {
        hookkit_status_t status = [self executeOperation:operation onBackend:nil];
        succeeded += status == HK_OK;
    }
    hookkit_status_t status = succeeded == operations.count ? HK_OK
