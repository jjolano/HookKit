...
// counter for unique hook/domain stable ids and symbol targets so the ONLY
// reason an op is ever rejected is plan state -- never a duplicate-id or
// duplicate-target INVALID_ARGUMENT, which would confound the model comparison.
...
static void test_failure_rollup_model(void) {
    // Prepare: one success plus one failure produces PARTIAL.
    {
        ...
        assert(hk_runtime_register_engine_for_testing(rt, &fake_rebind_engine));
        assert(hk_plan_create(rt, NULL, &plan) == HK_STATUS_OK);
        hk_hook_t *ok = add_model_hook(plan, "model.prepare.ok", "getpid");
        hk_hook_t *bad = add_model_hook(plan, "model.prepare.bad", "getppid");
    ...
    // Commit: one active plus one clean refusal produces PARTIAL.
    {
        ...
        assert(hk_runtime_register_engine_for_testing(rt, &fake_rebind_engine));
        assert(hk_plan_create(rt, NULL, &plan) == HK_STATUS_OK);
        hk_hook_t *ok = add_model_hook(plan, "model.commit.ok", "getpid");
        hk_hook_t *bad = add_model_hook(plan, "model.commit.bad", "getppid");
        assert(hk_plan_analyze(plan, NULL) == HK_STATUS_OK);
