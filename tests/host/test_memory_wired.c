...
#include "../../src/engines/HKMemoryVtable.h"
...
static void test_same_memory_target_fails_closed_across_plans(void) {
    uint8_t region[REGION];
    memcpy(region, ORIGINAL, REGION);
    ...
    hk_hook_spec_t first_spec = memory_spec("patch.first", (uintptr_t)region, false,
                                            ORIGINAL, REPLACEMENT);
    ...
    hk_report_t *report = NULL;
    assert(hk_plan_commit(first_plan, &report) == HK_STATUS_OK);
    ...
    hk_hook_spec_t second_spec = memory_spec("patch.second", (uintptr_t)region, false,
                                             REPLACEMENT, SECOND_REPLACEMENT);
    second_spec.target.memory.kind = HK_MEMORY_KIND_DATA;
    ...
    hk_runtime_release(rt);
    printf("  same-memory-target-fails-closed-across-plans: PASS\n");
}
...
int main(void) {
    ...
    RUN_TEST(test_same_memory_target_fails_closed_across_plans);
