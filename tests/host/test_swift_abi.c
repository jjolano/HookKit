...
// Host-side unit test for the Swift vtable engine (src/native/hk_swift.c). Runs
...
//
//   clang -Wall -Wextra -O2 -o /tmp/t tests/host/test_swift_abi.c && /tmp/t
//
...
// VM regions (src/native/hk_native.c, iOS-only). The host has no portable
