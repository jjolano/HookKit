...
// (src/core/HKEngineInternal.h, HKRuntime.c's
...
static void test_routes_on_original_requirement(void) {
    ...
    memset(&none_req.target.address, 0, sizeof(none_req.target.address));
    none_req.target.address.struct_size = sizeof(none_req.target.address);
    none_req.target.address.struct_version = HK_ABI_VERSION_3_0;
    none_req.target.address.address = 0x1000;
    none_req.original_requirement = HK_ORIGINAL_NONE;

    hk_hook_spec_t cont_req = none_req;
    cont_req.stable_hook_id = "h.cont";
    cont_req.target.address.address = 0x2000;
