...
static void bench_plan_e2e(int n, int iters, int warmup){
    ...
    for(int w=0;w<warmup;w++){
        ...
        hk_plan_t *plan=NULL; hk_plan_create(rt,NULL,&plan);
        for(int i=0;i<n;i++){ hk_hook_spec_t s=sym_spec(ids[i],"puts"); hk_hook_t *h=NULL; hk_plan_add_hook(plan,&s,&h); }
        hk_report_t *r=NULL;
        hk_plan_analyze(plan,&r); hk_report_release(r);
        hk_plan_prepare(plan,&r); hk_report_release(r);
        hk_plan_commit(plan,&r); hk_report_release(r);
    ...
    for(int i=0;i<iters;i++){
        hk_runtime_t *rt=NULL; hk_runtime_create(NULL,&rt);
        hk_plan_t *plan=NULL; hk_plan_create(rt,NULL,&plan);
        for(int k=0;k<n;k++){ hk_hook_spec_t s=sym_spec(ids[k],"puts"); hk_hook_t *h=NULL; hk_plan_add_hook(plan,&s,&h); }
        hk_report_t *r=NULL;
        uint64_t t0=now_ns();
        SIGNPOST_BEGIN("analyze");
        hk_plan_analyze(plan,&r);
        SIGNPOST_END("analyze");
        uint64_t t1=now_ns(); hk_report_release(r);
        SIGNPOST_BEGIN("prepare");
        hk_plan_prepare(plan,&r);
        SIGNPOST_END("prepare");
        uint64_t t2=now_ns(); hk_report_release(r);
        SIGNPOST_BEGIN("commit");
        hk_plan_commit(plan,&r);
        SIGNPOST_END("commit");
        uint64_t t3=now_ns(); hk_report_release(r);
        hk_plan_release(plan); hk_runtime_release(rt);
        analyze[i]=t1-t0;
        prepare[i]=t2-t1;
        commit[i]=t3-t2;
...
int main(int argc, char **argv){
    ...
    for(int i=1;i<argc;i++){
        ...
        else if(strcmp(argv[i],"--help")==0){ printf("usage: device_bench [--iters N] [--iters-e2e N] [--warmup N]\n"); return 0; }
    }
    ...
    for(size_t i=0;i<sizeof(Ns)/sizeof(Ns[0]);i++){
        // keep 1000 iters lower to avoid long device stall
