...
#include <stddef.h>
#include <stdint.h>
// This fixture shadows the platform <mach/mach.h> whenever the test build
// puts tests/fixtures/native-vm first on the include path. It declares the
// stub ABI unconditionally: the test file defines hk_test_* bodies and
// hk_native.c calls through the renames below, so deferring to real headers
// would split the TU between real types and stub bodies. Include-order
// discipline (fixture first, real pthread.h only via hk_native.c after the
// fixture won) keeps the real mach_port_t from ever entering this TU.
