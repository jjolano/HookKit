...
[ "$actual_version" = 3.0.4-1 ] || { echo "FAIL version '$actual_version' != '3.0.4-1'" >&2; exit 1; }
...
check_gum_package() {
    ...
    [ "$actual_version" = 3.0.4-1 ] || {
        echo "FAIL Gum version '$actual_version' != '3.0.4-1'" >&2; return 1;
    }
    printf '%s\n' "$depends" | grep -Fq 'me.jjolano.fmwk.hookkit (= 3.0.4-1)' || {
