...
foreach my $line (@lines) {
	...
	while($line =~ m/\B(?=(\%\w|&\s*\%\w|[+-]\s*\(\s*.*?\s*\)))/gc) {
		...
		if($line =~ /\G%hook\s+([\$_\w\.]+)/gc) {
			...
			# hookkit: per-method inheritance opt-in via __attribute__((annotate("hookkit:allow_inherited")))
			# ponytail: detect before selector parsing; strip so selector parse doesn't see __attribute__ as keyword
			# save pos because s/// resets pos() in perl
			my $hk_save_pos = pos($line);
			if (index($line, "hookkit:allow_inherited") != -1) {
				$method->{_hookkit_allow_inherited} = 1;
				# preserve string length so brace depth mapping (computed pre-strip) stays aligned — replace with spaces
				$line =~ s/__attribute__\s*\(\s*\(\s*annotate\s*\(\s*\"hookkit:allow_inherited\"\s*\)\s*\)\s*\)/" " x length($&)/ge;
				$line =~ s/__attribute__\s*\(\s*\(\s*\"hookkit:allow_inherited\"\s*\)\s*\)/" " x length($&)/ge;
				$line =~ s/\bhookkit:allow_inherited\b/" " x length($&)/ge;
				# restore pos so \G selector loop continues at saved offset (preserve-length keeps offset valid)
				pos($line) = $hk_save_pos if defined $hk_save_pos;
			}
