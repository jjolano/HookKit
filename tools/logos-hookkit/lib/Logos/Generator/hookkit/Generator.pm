...
sub staticDeclarations {
    ...
    return join("\n", ($self->SUPER::staticDeclarations(),
        ...
        "}",
        "__attribute__((unused)) static int _hk_hookkit_hook_message(const char *_stable_hook_id, Class _cls, SEL _sel, IMP _new, IMP *_old, int _isMeta, int _allowInherited) {",
        ...
        "    if (_allowInherited) _t.inheritance_policy = HK_OBJC_ALLOW_INHERITED_OVERRIDE;",
        ...
        "    _spec.original_requirement = _old ? HK_ORIGINAL_DIRECT_PREDECESSOR : HK_ORIGINAL_NONE;",
        "    hk_hook_t *_hook = NULL; if (hk_plan_add_hook(_plan, &_spec, &_hook) != HK_STATUS_OK) { hk_plan_release(_plan); return -1; }",
        "    if (hk_plan_analyze(_plan, NULL) != HK_STATUS_OK) { hk_plan_release(_plan); return -1; }",
        "    hk_hook_result_t _prepared; memset(&_prepared,0,sizeof(_prepared)); _prepared.struct_size=sizeof(_prepared); _prepared.struct_version=HK_ABI_VERSION_3_0;",
        "    if (hk_hook_copy_result(_hook,&_prepared)!=HK_STATUS_OK || _prepared.outcome!=HK_OUTCOME_ANALYZED) { hk_plan_release(_plan); return -1; }",
        "    if (hk_plan_prepare(_plan, NULL) != HK_STATUS_OK) { hk_plan_release(_plan); return -1; }",
        "    if (hk_hook_copy_result(_hook,&_prepared)!=HK_STATUS_OK || _prepared.outcome!=HK_OUTCOME_PREPARED) { hk_plan_release(_plan); return -1; }",
        "    if (_old) { if (!_prepared.continuation.address) { hk_plan_release(_plan); return -1; } *_old=(IMP)_prepared.continuation.address; }",
        "    if (hk_plan_commit(_plan, NULL) != HK_STATUS_OK) { hk_plan_release(_plan); return -1; }",
        ...
        "            _ok=0; if (_old) { void *_o=(void*)hk_original_slot_load(hk_hook_original_slot(_hook)); if(!_r.original_available || !_o) { _ok=-1; } else if (_o != (void*)_prepared.continuation.address) *_old=(IMP)_o; }",
        "        }",
        "    }",
