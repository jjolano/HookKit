...
# or one lane:
bash tools/release/install-theos.sh rootless
```
...
bash tools/release/check_exports.sh # packaging/abi/HookKit.tbd unchanged; Compat.h adds no exports
```

Device: `tests/device/device_lifecycle_smoke.c` /
`tests/device/device_objc_smoke.m` pattern — runtime + plan + orig slot.
