...
// HookKitPlan.h). Shared across src/core/*.c.
...
struct hk_hook {
    ...
    // Canonical target identity shared with the ownership ledger. Keeping one
    // owned key per hook makes duplicate-plan admission and commit grouping
    // linear rather than rebuilding keys for every comparison.
    uint8_t *target_key;
