...
static bool hk_hooks_target_equal(const struct hk_hook *left,
                                  const struct hk_hook *right) {
    return left && right && left->target_key && right->target_key &&
           left->target_key_size == right->target_key_size &&
           memcmp(left->target_key, right->target_key,
                  left->target_key_size) == 0;
}
    ...
    hk_hook_t **out_hook)
{
    ...
    if (!hk_ownership_target_key_copy(&hook->spec, &hook->target_key,
                                      &hook->target_key_size)) {
        hk_hook_free(hook);
        return HK_STATUS_OUT_OF_MEMORY;
    }
    for (size_t i = 0; i < plan->hook_count; i++) {
        if (hk_hooks_target_equal(plan->hooks[i], hook)) {
            // Sibling hooks prepare against the same pre-commit state, so
            // they cannot form a reliable chain. Deliberate chaining uses
            // separate plans, where each successor sees a published head.
            hk_hook_free(hook);
            return HK_STATUS_INVALID_ARGUMENT;
        }
    }
...
static size_t hk_plan_commit_group_count(const hk_plan_t *plan,
    ...
    for (size_t offset = 1; start + offset < commit_count; offset++) {
        ...
        for (size_t previous = start; previous < start + offset; previous++) {
            if (hk_hooks_target_equal(plan->hooks[commit_order[previous]],
                                      hook)) {
                return count > 1 ? count : 0;
            }
        }
