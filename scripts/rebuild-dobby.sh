...
# ignored candidate archive under build/; replacing vendor/dobby/libdobby.a
...
"$LIPO" -create "$TMP/build-arm64/libdobby.a" "$TMP/build-arm64e/libdobby.a" \
    -output "$REBUILT"
check_shape "$REBUILT"
mkdir -p "$ROOT/build/dobby-rebuild"
cp "$REBUILT" "$ROOT/build/dobby-rebuild/libdobby.a"
echo "Candidate: build/dobby-rebuild/libdobby.a ($(sha256 "$REBUILT"))"
