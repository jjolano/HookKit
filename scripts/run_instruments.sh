...
# Usage: bash scripts/run_instruments.sh <UDID|device-name> [--iters-e2e N]
...
if [[ "$(uname -s)" != "Darwin" ]]; then
  ...
  echo "  bash scripts/run_device_bench.sh <host> && grep signpost" >&2
  exit 0
fi
...
OUT="artifacts/trace_$(date +%Y%m%d_%H%M%S).trace"
