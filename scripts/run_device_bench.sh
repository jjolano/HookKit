...
# Usage: bash scripts/run_device_bench.sh [user@host] [--iters N] [--iters-e2e N]
...
ssh "$DEVICE_SSH" "/tmp/device_bench ${EXTRA_ARGS[*]:-}" | tee "artifacts/device_bench_$(date +%Y%m%d_%H%M%S).log"
# Also capture pure JSON for baseline
# ssh "$DEVICE_SSH" "/tmp/device_bench ${EXTRA_ARGS[*]:-}" | grep '^{' > artifacts/device_bench_baseline.json || true
# echo "JSON baseline at artifacts/device_bench_baseline.json"
