#!/usr/bin/env python3
"""Validate a shadow-hook-manifest.json against Schemas/shadow-hook-manifest.schema.json.
...
def main():
    ...
    ap.add_argument("manifest", nargs="?",
                     default=None,
                     help="manifest JSON to validate (default: artifacts/shadow-current-manifest.json)")
    args = ap.parse_args()

    repo_root = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", ".."))
    manifest_path = args.manifest or os.path.join(repo_root, "artifacts", "shadow-current-manifest.json")
    schema_path = os.path.join(repo_root, "Schemas", "shadow-hook-manifest.schema.json")
