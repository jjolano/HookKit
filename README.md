...
The Gum package is intentionally absent from the legacy rootful lane. Its package dependency requires iOS 15 or newer in every modern lane, including modern rootful (whose core package remains iOS 14+). If it is installed or removed while a host process is running, restart that process: provider discovery is memoized. Inline patching needs relaxed codesigning (same constraint as native/Dobby). Hooks should still be installed at load time, but Frida is deliberately **not** main-thread gated the way native and Dobby are: Gum ships a production relocator and interceptor transactions, so an off-main-thread inline hook is better served by falling through to it than refused outright. The required base package owns the version-pinned [Frida Gum SBOM](layout/usr/share/doc/hookkit/Frida-Gum-17.17.0-SBOM.md) and license texts, so every installed Gum provider has the notices without duplicate package paths.
...
Every lane runs `make clean`, the existing host tests, package verification, and export verification in isolated staging. `scripts/check_compat.sh` verifies the core notice tree and, on modern lanes, the separate Gum-only payload. `scripts/rebuild-dobby.sh --check` verifies Dobby's locked archive and source patch. Modern arm64e release builds require macOS with Xcode 12 or newer. The legacy lane defaults to `$THEOS/toolchain/oldabi/linux/iphone` and `$THEOS/sdks/iPhoneOS13.7.sdk`; `OLDABI_TOOLCHAIN` and `OLDABI_SDKS` remain optional overrides. RootHide requires the RootHide Theos fork. GitHub Actions builds the legacy lane on Linux and all modern lanes on macOS.
